...
fn vertexMain(input: VertexInput) -> VertexOutput {
    var output: VertexOutput;
    let angle = uMyUniforms.time * 0.02;
    let c1 = cos(angle);
    let s1 = sin(angle);
    let ratio = 1.0;
    var position = vec3f(
    ...
    //output.pos = transpose(uMyUniforms.projectionMatrix) * transpose(uMyUniforms.viewMatrix) * transpose(uMyUniforms.modelMatrix) * vec4f(input.pos, 1.0);
    output.col = input.col;
    return output;
}
